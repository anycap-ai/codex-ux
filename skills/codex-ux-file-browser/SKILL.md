...
Preferred fast path: run the Python launcher from the workspace root the user wants to review:

```bash
python3 ~/.codex/skills/codex-ux-file-browser/scripts/launch.py "$PWD" --reuse --detach --json
```
...
```bash
python3 ~/.codex/skills/codex-ux-file-browser/scripts/launch.py "$PWD"
```

Use a fixed port only when the user explicitly asks for one:

```bash
python3 ~/.codex/skills/codex-ux-file-browser/scripts/launch.py "$PWD" --reuse --detach --json --port 8790
```
